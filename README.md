...
graph TB
    subgraph 数据处理阶段_离线
        A1[RGB-D图像输入] --> A2[拉普拉斯模糊过滤]
        A2 --> A3[Owl-Vit & DINO<br/>物体识别]
        A3 --> A4[SAM 精细分割]
        A4 --> A5[2D掩码→3D点云映射]
        A5 --> A6[空间关系计算<br/>距离/方位]
        A6 --> A7[VLM 颜色材质分析]
        A7 --> A8[(Neo4j<br/>场景知识图谱)]
        A7 --> A9[(ConceptNet<br/>世界知识库)]
    end
    ...
    subgraph 问答推理阶段_在线
        ...
        B4 -->|空间关系| B5[检索物体空间关系]
        B4 -->|状态问题| B6[决定读取图片数量<br/>状态:10张 非状态:5张]
        B4 -->|意图整理| B7[整理场景所有物体]
        ...
        B8 --> B9[关键词世界知识检索<br/>ConceptNet]
        B8 --> B10[关键词图片采样<br/>Neo4j物体库]
        
        B10 --> B11{知识库是否<br/>有信息?}
        
        B11 -->|有| B12[场景信息<br/>+<br/>物体图片]
        B11 -->|无| B13[场景图采样<br/>均匀采样50帧]
        
        B12 --> B14[纵向Top-N选帧<br/>VLM锦标赛]
        B13 --> B14
        
        B14 --> B15[综合Top-K回答<br/>候选答案生成]
        ...
        B17 --> B18{最高分<0.7?}
        B18 -->|是| B19[进入二分间隔重采样]
        B18 -->|否| B20[选择Top-K进行锦标赛<br/>竞争出Top-2]
        
        B19 --> B13
        B20 --> B21[最终答案生成]

        
        B21 --> B22[输入Top-2候选<br/>生成最终答案]
        
        B22 --> B23[答案有效性裁判]
        B23 --> B24{答案是否<br/>有意义?}
        
        B24 -->|无意义| B25[无法判断/不存在/看不见<br/>进入重采样回答]
        B24 -->|有意义| B26[输出最终答案]
    ...
    style A9 fill:#fff4e1
    style B26 fill:#e8f5e9
    style B25 fill:#ffebee
```
