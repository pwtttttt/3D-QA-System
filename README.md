...
本系统是一个基于多模态大语言模型的三维场景问答系统，专门针对三维、二维数据集设计。系统通过问题解析、等间隔帧序列分析、知识库检索、答案过滤、最终生成和有效性裁判六个核心阶段，实现了对三维场景视觉问题的智能回答。系统采用分层采样策略、锦标赛选择机制和多轮重采样兜底逻辑，确保了答案的准确性和鲁棒性。
...
### 1.1 项目背景
随着三维场景数据的快速增长，三维空间视觉理解的需求日益迫切。传统的问答系统主要针对二维图像设计，而本系统针对三维场景的复杂视觉问答任务进行了优化。
