...
### 1.1 项目背景
本系统是一个基于多模态大语言模型的三维场景问答系统，专门针对三维、二维数据集设计。本项目构建了一套通用的 RGB-D 视觉处理流水线，集成了 yolo-world (检测)、SAM 2(分割)、CLIP (特征对齐) 及 VLM (**大模型**推理) 等前沿技术实现了空间场景的信息提取。系统通过问题解析、等间隔帧序列分析、知识库检索、答案过滤、最终生成和有效性裁判六个核心阶段，实现了对三维场景视觉问题的智能回答。系统采用分层采样策略、锦标赛选择机制和多轮重采样兜底逻辑，确保了答案的准确性和鲁棒性。
